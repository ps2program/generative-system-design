...
# orchestrated sub level handlers

# requirement, multithread
# def handle_requirements_question(state):
#     """
#     Generate answers for requirements questions concurrently and store the results in the database.
...
#     lock = threading.Lock()
#     request_count = 0
#     all_added_requirements = []

#     def process_requirement(question):
...
#             # Upload requirements
#             added_requirements = orchestrator.batch_top_level_requirement_upload(
#                 json.loads(response.content)
#             )
#             all_added_requirements.extend(added_requirements)

#         except Exception as e:
#             print(f"Error processing question '{question}': {e}")

#     # Use a thread pool to process requirements concurrently
#     with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
#         executor.map(process_requirement, [question])  # Currently a single question, expand if needed

#     # Create the detailed requirements data for the response schema
#     detailed_requirements = []
#     for requirement in all_added_requirements:
#         requirement_data = {
#             "id": str(requirement.id),
#             "data": requirement.data,
#             "parent_requirements": [
#                 {"id": parent.id, "data": parent.data} for parent in requirement.parent_requirements
#             ],
#             "sub_requirements": [
#                 {"id": sub.id, "data": sub.data} for sub in requirement.sub_requirements
#             ],
#         }
#         detailed_requirements.append(requirement_data)

#     # Create response schema
#     agent_state = {
#         "messages": ["Requirements processed successfully."],
#         "response_schema": create_schema(str(detailed_requirements), "Requirements_Handler"),
#     }
...
def handle_requirements_question(state):
    ...
    # response = model.invoke(conversation_history) old approch with total history
    response = model.invoke([nextPrompt])

    #orchestrator updates the state memory
    requirements_added = orchestrator.batch_top_level_requirement_upload(json.loads(response.content))
...
#2. streaming with multithread worker = 5
def handle_functions_question(state):
    """
    Generate functions for each requirement and publish updates via PubSubManager.

    Args:
        state (dict): The current state containing messages and other context.

    Returns:
        dict: The updated state with the generated functions.
    """
    print("---GENERATE FUNCTIONS ANSWERS---")

    # Fetch all requirements from state_memory
    try:
        requirements = orchestrator.get_all_requirements()
        if not requirements:
            pubsub_manager.publish({"error": "No requirements found to generate functions."})
            return {"error": "No requirements found to generate functions."}
    except Exception as e:
        pubsub_manager.publish({"error": f"Error fetching requirements: {e}"})
        return {"error": f"Error fetching requirements: {e}"}
    ...
    def process_requirement(requirement):
        ...
        requirement_title = requirement['data']['title']
        ...
        try:
            ...
            next_prompt = HumanMessage(
                content=f"""
                You are a mechanical designer. Your task is to generate a list of Functions
                for the following requirement: "{requirement_title}". 
                ...
                Ensure that you only provide the list of functions. Do not include any additional commentary or explanations. Each list should have maximum of two functions not more than that.
                """
            )
            model = get_global_model()
            response = model.invoke([next_prompt])
            functions_data = json.loads(response.content)

            # Upload functions
            added_functions = orchestrator.batch_function_upload(requirement_id, functions_data)
            all_added_functions.extend(added_functions)
            ...
            for function in added_functions:
                function_data = {
                    ...
                    "requirements": [str(req.id) for req in function.requirements],
                }
                pubsub_manager.publish(function_data)

        except Exception as e:
            pubsub_manager.publish({"error": f"Error processing requirement {requirement_id}: {e}"})

    # Process requirements concurrently
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        executor.map(process_requirement, requirements)

    # Create the detailed functions data for the response schema
    detailed_functions = []
    for function in all_added_functions:
        function_data = {
            "id": str(function.id),  # Convert UUID to string
            "data": function.data,
            "parent_functions": [
                {"id": str(parent.id), "data": parent.data} for parent in function.parent_functions  # Convert UUID to string
            ],
            "sub_functions": [
                {"id": str(sub.id), "data": sub.data} for sub in function.sub_functions  # Convert UUID to string
            ],
            "requirements": [
                {"id": str(req.id), "data": req.data} for req in function.requirements  # Convert UUID to string
            ],
        }
        detailed_functions.append(function_data)
    # Create response schema
    agent_state = {
        "messages": [json.dumps(detailed_functions, indent=2)],
        "response_schema": create_schema(str(detailed_functions), "Function_Handler"),
    }
...
# def handle_functions_question(state):
#     """
#     Generate functions for each requirement retrieved from the state memory
#     and store the results using the Orchestrator.
...
#     # Fetch all requirements from state_memory using the Orchestrator
#     try:
#         requirements = orchestrator.get_all_requirements()
#         if not requirements:
#             print("No requirements found in the state memory.")
#             return {"error": "No requirements found to generate functions."}
#     except Exception as e:
#         print(f"Error fetching requirements: {e}")
#         return {"error": "Error fetching requirements from state memory."}

#     # Initialize a list to hold all added functions
#     all_added_functions = []

#     # Iterate through each requirement and generate functions for it
#     for requirement in requirements:
#         requirement_id = requirement['id']
#         requirement_title = requirement['data']['title']

#         print(f"Generating functions for requirement: {requirement_title} (ID: {requirement_id})")

#         # Create the prompt for the LLM to generate functions
#         next_prompt = HumanMessage(
#             content=f"""
#                 You are a mechanical designer. Your task is to generate a list of Functions
#                 for the following requirement: "{requirement_title}". 
...
#     # Each list should have maximum of two functions, not more than that.

#         # Fetch conversation history and append the prompt
#         conversation_history = get_conversation_history()
#         conversation_history.append(next_prompt)

#         # Get the global model and invoke it to generate functions
#         try:
#             model = get_global_model()
#             response = model.invoke([next_prompt])
...
#                 raise ValueError("The LLM response is not a valid list of functions.")

#             # Upload the functions to state memory using the Orchestrator
#             added_functions = orchestrator.batch_function_upload(requirement_id, functions_data)
#             all_added_functions.extend(added_functions)

#             print(f"Functions added for requirement ID {requirement_id}: {added_functions}")

#         except json.JSONDecodeError as e:
#             print(f"Error decoding LLM response for requirement {requirement_id}: {e}")
#         except Exception as e:
#             print(f"Error processing functions for requirement ID {requirement_id}: {e}")
...
#     agent_state = {
#         "messages": [response],
#         "response_schema": create_schema(str(detailed_functions), "Functions_Handler")
#     }
    
#     return agent_state

# 2. product multithread with streaming
def handle_products_question(state):
    """
    Generate components (physicals) for each function retrieved from the state memory
    and store the results using the Orchestrator, with a maximum of 50 LLM requests per minute.

    Args:
        state (dict): The current state containing messages and other context.

    Returns:
        dict: The updated state with the generated components (physicals).
    """

    print("---GENERATE COMPONENTS (PHYSICALS) ANSWERS---")

    # Fetch all functions from state_memory using the Orchestrator
    try:
        functions = orchestrator.get_all_functions()
        if not functions:
            pubsub_manager.publish({"error": "No functions found to generate components (physicals)."})
            return
    except Exception as e:
        pubsub_manager.publish({"error": f"Error fetching functions: {e}"})
        return
    ...
    def process_function(function):
        ...
        function_title = function['data']['title']
        ...
        try:
            ...
            # Create the prompt for generating components
            next_prompt = HumanMessage(
                content=f"""
                    You are a mechanical designer. Your task is to generate a list of **Components (Physicals)**
                    for the following function: "{function_title}". 

                    Your response should be an array of JSON objects in the following format:
                    [
                        {{"index": "1", "title": "Component Title", "description": "Description of the Component"}}
                    ]

                    Ensure that you only provide the list of components (physicals). Do not include any additional commentary or explanations. Each list should have a maximum of two components.
                """
            )
            ...
            # Parse the response as JSON
            physicals_data = json.loads(response.content)
            if not isinstance(physicals_data, list):
                raise ValueError("The LLM response is not a valid list of components (physicals).")

            # Upload components to state memory
            added_physicals = orchestrator.batch_physical_upload_to_function(function_id, physicals_data)
            all_added_physicals.extend(added_physicals)

            for physical in added_physicals:
                physical_data = {
                    "id": str(physical.id),
                    "data": physical.data,
                    "functions": [str(func.id) for func in physical.functions],
                    "sub_physicals": [str(sub.id) for sub in physical.sub_physicals],
                    "requirements": [str(req.id) for req in physical.requirements],
                }
                pubsub_manager.publish(physical_data)

        except Exception as e:
            pubsub_manager.publish({"error": f"Error processing function {function_id}: {e}"})

    # Use ThreadPoolExecutor to process functions concurrently
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        executor.map(process_function, functions)

    # Create the detailed components (physicals) data for the response schema
    detailed_physicals = []
    for physical in all_added_physicals:
        physical_data = {
            'id': str(physical.id),
            'data': physical.data,
            'functions': [
                {'id': str(func.id), 'data': func.data} for func in physical.functions
            ],
            'sub_physicals': [
                {'id': str(sub_phys.id), 'data': sub_phys.data} for sub_phys in physical.sub_physicals
            ],
            'requirements': [
                {'id': str(req.id), 'data': req.data} for req in physical.requirements
            ]
        }
        detailed_physicals.append(physical_data)

    # Create response schema
    agent_state = {
        "messages": [json.dumps(detailed_physicals, indent=2)],
        "response_schema": create_schema(str(detailed_physicals), "Component_Handler")
    }
...
#     # Fetch all functions from state_memory using the Orchestrator
#     try:
#         functions = orchestrator.get_all_functions()  # Fetch functions from state memory
#         if not functions:
#             print("No functions found in the state memory.")
#             return {"error": "No functions found to generate components (physicals)."}
#     except Exception as e:
#         print(f"Error fetching functions: {e}")
#         return {"error": "Error fetching functions from state memory."}

#     # Initialize a list to hold all added components (physicals)
#     all_added_physicals = []

#     # Iterate through each function and generate components (physicals) for it
#     for function in functions:
#         function_id = function['id']
#         function_title = function['data']['title']

#         print(f"Generating components (physicals) for function: {function_title} (ID: {function_id})")

#         # Create the prompt for the LLM to generate components (physicals)
#         next_prompt = HumanMessage(
#             content=f"""
#                 You are a mechanical designer. Your task is to generate a list of **Components (Physicals)**
#                 for the following function: "{function_title}". 
...
#                 Ensure that you only provide the list of components (physicals). Do not include any additional commentary or explanations. Each list should have maximum two physicals(components), not more than that.
#             """
#         )
...
#         # Get the global model and invoke it to generate components
#         try:
#             model = get_global_model()
#             response = model.invoke([next_prompt])
...
#             # Upload the components to state memory using the Orchestrator
#             added_physicals = orchestrator.batch_physical_upload_to_function(function_id, physicals_data)
#             all_added_physicals.extend(added_physicals)

#             print(f"Components (physicals) added for function ID {function_id}: {added_physicals}")

#         except json.JSONDecodeError as e:
#             print(f"Error decoding LLM response for function {function_id}: {e}")
#         except Exception as e:
#             print(f"Error processing components (physicals) for function ID {function_id}: {e}")
...
#             'data': physical.data,
#             'functions': [
#                 {'id': func.id, 'data': func.data} for func in physical.functions
#             ],
#             'sub_physicals': [
#                 {'id': sub_phys.id, 'data': sub_phys.data} for sub_phys in physical.sub_physicals
#             ],
#             'requirements': [
#                 {'id': req.id, 'data': req.data} for req in physical.requirements
#             ]
#         }
#         detailed_physicals.append(physical_data)
...
#     agent_state = {
#         "messages": [response],
#         "response_schema": create_schema(str(detailed_physicals), "Components_Handler")
#     }
